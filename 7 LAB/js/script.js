...
$(document).ready(function () {
    ...
    function TableYchastki() {
        ...
        if (a.length > 0) {
            for (let i = 0; i < a.length; i++) {
                let row = a[i];
                let matchAll = row.matchAll(/^\d*/gm);
                matchAll = Array.from(matchAll);
                let firstMatch = matchAll[0];

                let indx = firstMatch[0].length;
                let element1 = `ploshad${i + 1}`;
                let element2 = `koord${i + 1}`;
                let str = row.substr(indx + 1, row.length);
                $(`#${element1}`).val(firstMatch[0]);
                $(`#${element2}`).val(str);
            }
        }
    ...
    function abnormalHide() {
        ...
        for (let i = 1; i <= n; i++) {
            ...
            if (parseFloat(document.getElementById(element1).value) < minv) {
                minv = parseFloat(document.getElementById(element1).value);
...
function CanvDraw() {
    ...
    let max = k * Otklonenie.value + +SredPloshad.value;
    ...
    var koord;
    let min = SredPloshad.value - k * Otklonenie.value;
    ...
    $(canv).css("background-color", inputForm['backColorInput']);

    ctx.strokeStyle = inputForm['colorAXIS'];
    ctx.lineWidth = 1;
    ctx.moveTo(5.5, 5.5);
    ctx.lineTo(3.5, 15.5);
    ctx.moveTo(5.5, 5.5);
    ctx.lineTo(7.5, 15.5);
    ctx.moveTo(5.5, 5.5);
    ctx.fillStyle = inputForm['fontColorInput'];
    ...
    ctx.strokeStyle = inputForm['colorMark'];
    ...
    ctx.beginPath();
    ctx.strokeStyle = inputForm['colorLine_abno']
    ...
    ctx.beginPath();
    ctx.strokeStyle = inputForm['colorLine_mid']
    ...
    ctx.beginPath();
    ctx.strokeStyle = inputForm['colorLine_dev'];
    ...
    ctx.fillText('Аномаль.', 570.5 * min / maxv, 10.5);
    ctx.fillText(min.toFixed(2), 570.5 * min / maxv, 22.5);
...
function map() {
    ...
    pic.onload = function () {
        ...
        for (var i = 1; i <= n; i++) {
            ...
            ctx.beginPath();
            ctx.fillStyle = `rgba(${20 * i}, ${87 + i * 20}, ${37 - i}, 0.7)`;
            ...
            if ((document.getElementById(element1).value - SredPloshad.value) / Otklonenie.value >= k ||
                (document.getElementById(element1).value - SredPloshad.value) / Otklonenie.value <= -k) {
